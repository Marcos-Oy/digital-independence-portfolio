...
const AgentContextRow = ({ icon: Icon, label, connected = true }: { icon: React.ComponentType<{ className?: string }>; label: string; connected?: boolean }) => (
  <div className="flex items-center gap-2.5">
    ...
    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${connected ? "bg-secondary" : "bg-border"}`} aria-hidden="true" />
  </div>
);

/** Panel del hero: laptop con agente conectado a contexto del negocio + teléfono con flujos activos. */
const HeroAgentPanel = () => (
  <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
    <LaptopMock className="shadow-card-hover">
      ...
      <div className="space-y-3">
        <AgentContextRow icon={BookOpenText} label="Documentos" />
        <AgentContextRow icon={Database} label="Historial de clientes" />
        <AgentContextRow icon={Workflow} label="Flujos de trabajo" />
        <AgentContextRow icon={Plug} label="Integraciones" connected={false} />
      </div>
    </LaptopMock>
    <PhoneMock className="w-24 shadow-card hidden sm:block">
      ...
      <div className="space-y-1.5">
        <span className="block text-[8px] font-semibold text-secondary bg-secondary/10 rounded px-1.5 py-1">Activa</span>
        <span className="block text-[8px] font-semibold text-primary bg-primary/8 rounded px-1.5 py-1">Con memoria</span>
        <span className="block text-[8px] font-semibold text-foreground/70 bg-muted rounded px-1.5 py-1">Conectada</span>
      </div>
    </PhoneMock>
  </div>
);

/** Panel del problema: herramientas de IA aisladas, sin contexto del negocio. */
const DisconnectedAiPanel = () => {
  const rows = [
    { icon: Bot, label: "IA genérica", connected: true },
    { icon: BookOpenText, label: "Documentos", connected: false },
    { icon: Database, label: "Historial", connected: false },
    { icon: Workflow, label: "Flujo A", connected: true },
    { icon: Plug, label: "CRM", connected: false },
    { icon: MessagesSquare, label: "Mensajería", connected: false },
  ];
  return (
        ...
        {rows.map((r) => (
          <div
            ...
            className={`flex items-center gap-2 rounded-lg border px-2.5 py-2 ${
              r.connected ? "border-border bg-muted/40" : "border-dashed border-border/70"
            }`}
          >
...
/** Panel de solución: laptop + monitor con ecosistema de IA conectado y activo. */
const ConnectedAiPanel = () => (
  <div className="grid grid-cols-2 gap-3">
    <LaptopMock className="col-span-2 shadow-card-hover">
      <p className="text-[10px] font-heading font-bold uppercase tracking-wide text-muted-foreground mb-3">
        Ecosistema de IA conectado
      </p>
      <div className="space-y-3">
        <AgentContextRow icon={BookOpenText} label="Base de conocimiento" />
        <AgentContextRow icon={Workflow} label="Automatización de flujos" />
        <AgentContextRow icon={Plug} label="Herramientas integradas" />
      </div>
    </LaptopMock>
    <div className="col-span-2 rounded-xl border border-border bg-muted/60 px-4 py-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="text-[9px] font-semibold text-secondary bg-secondary/10 rounded-full px-2.5 py-1">Con contexto</span>
        <span className="text-[9px] font-semibold text-primary bg-primary/8 rounded-full px-2.5 py-1">En producción</span>
      </div>
    </div>
  </div>
);

/** Panel de resultado: agentes y automatizaciones activos. */
const ResultPanel = () => (
  <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
    <LaptopMock className="shadow-card-hover">
      <p className="text-[10px] font-heading font-bold uppercase tracking-wide text-muted-foreground mb-3">
      ...
      <div className="space-y-2.5">
        <AgentContextRow icon={Bot} label="Agente de atención" />
        <AgentContextRow icon={Workflow} label="Flujo de seguimiento" />
        <AgentContextRow icon={UserCircle2} label="Clon digital" />
        <AgentContextRow icon={MessagesSquare} label="Conversaciones" />
        <AgentContextRow icon={Plug} label="Nueva integración" connected={false} />
      </div>
    </LaptopMock>
    <PhoneMock className="w-24 shadow-card hidden sm:block">
      <p className="text-[8px] font-heading font-bold uppercase tracking-wide text-muted-foreground mb-2">
...
const CtaPanel = () => (
  <div className="rounded-2xl border border-brand-foreground/20 bg-brand-foreground/10 backdrop-blur-sm p-5 md:p-6">
    ...
    <div className="space-y-3">
      {["Diagnóstico", "Diseño", "Entrenamiento", "Implementación"].map((step, i) => (
